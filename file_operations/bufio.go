...
func main() {
	...
	file, err := os.OpenFile("test_file.txt", os.O_WRONLY|os.O_CREATE, 0644)
	// error handling
	if err != nil {
		log.Fatal(err)
	}
	...
	// => 2020/12/03 11:40:35 Bytes written to buffer (not file): 3

	// checking the available buffer
	bytesAvailable := bufferedWriter.Available()
	log.Printf("Bytes available in buffer: %d\n", bytesAvailable)
	// => 2020/12/03 11:40:35 Bytes available in buffer: 4093
